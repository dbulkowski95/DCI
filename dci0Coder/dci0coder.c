...
//		printf("Wartosc na wyjsciu: %u\n", u.outputValue);
	...
	else if (strcmp(argv[1], "dci1") == 0)
	{

	}

	else if (strcmp(argv[1], "dci60a") == 0)
	{
	}
	else
	{
		dci0OutputValue AnnoymousUnionOfDci0Value;
		dci0InputS dciInput;
		uint32_t tempDci[amountOfArgumentsInDci0Output];
		...
		argAssigmentForDci0(&dciInput, tempDci);
		argAssigmentForDci0Output(&dciInput, bandwidth, tempDci);
		argAssigmentForDci0OutputToStruct(&AnnoymousUnionOfDci0Value, tempDci);
		printFunctionToDci0(&AnnoymousUnionOfDci0Value);
	}
	return 0;
}
